...
TOP_CAMERA_INDEX="${TOP_CAMERA_INDEX:-${TOP_CAM_INDEX:-4}}"
WRIST_CAMERA_INDEX="${WRIST_CAMERA_INDEX:-${WRIST_CAM_INDEX:-6}}"
CAMERA_WIDTH="${CAMERA_WIDTH:-640}"
CAMERA_HEIGHT="${CAMERA_HEIGHT:-480}"
CAMERA_FPS="${CAMERA_FPS:-30}"
CAMERA_WARMUP_S="${CAMERA_WARMUP_S:-1}"

DATASET_REPO_ID="${DATASET_REPO_ID:-local/eval_act_so101}"
DATASET_ROOT="${DATASET_ROOT:-./outputs/eval_act_so101}"
DATASET_FPS="${DATASET_FPS:-${FPS:-${CAMERA_FPS}}}"
NUM_EPISODES="${NUM_EPISODES:-1}"
EPISODE_TIME_S="${EPISODE_TIME_S:-${RUN_TIME_S:-300}}"
...
echo "Camera indexes bridged to guarded flags: top=${TOP_CAMERA_INDEX}, wrist=${WRIST_CAMERA_INDEX}"
echo "Control FPS bridged to dataset_fps: ${DATASET_FPS}"
echo "Run time bridged to episode_time_s: ${EPISODE_TIME_S}s"
