...
OUTPUT_DIR=${OUTPUT_DIR:-/data/tfj/lerobot_tfj/outputs/act_distill_grasp_block_in_bin1_stage2_smoke_20260315_153740}

"${PYTHON_BIN}" -m lerobot.scripts.lerobot_train \
  --config_path="${BASE_CONFIG}" \
  --output_dir="${OUTPUT_DIR}" \
  --job_name=act_grasp_block_in_bin1_stage2_smoke \
  --batch_size=4 \
  --steps=4 \
  --num_workers=2 \
  --log_freq=1 \
  ...
  --policy.decoder_kd.peak_weight=0.05 \
  ...
  --policy.decoder_kd.start_step=0 \
  --policy.decoder_kd.ramp_steps=0 \
