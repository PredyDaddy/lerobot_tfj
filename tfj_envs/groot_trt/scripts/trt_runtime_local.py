...
class TrtSession:
    def __init__(self, engine_path: str | Path, device: str = "cuda:0") -> None:
        ...
        if self.device.type != "cuda":
            raise ValueError(f"TrtSession requires a CUDA device, got {self.device}")
    ...
    def run(self, feed_dict: Mapping[str, np.ndarray | torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        with torch.cuda.device(self.device), torch.cuda.stream(self.stream):
            ...
            for name in self.input_names:
                ...
                if any(int(dim) < 0 for dim in self.engine.get_tensor_shape(name)):
