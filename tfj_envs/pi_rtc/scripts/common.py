...
def build_processor_overrides(
    *,
    device: str = "cpu",
    local_tokenizer_path: str | Path | None = None,
    override_tokenizer_name: bool = True,
) -> dict[str, Any]:
    overrides: dict[str, Any] = {
        "device_processor": {"device": device},
    }
    if override_tokenizer_name and local_tokenizer_path is not None:
...
def resolve_pipeline_tokenizer_name(pipeline: Any) -> str | None:
    for step in getattr(pipeline, "steps", []):
        tokenizer_name = getattr(step, "tokenizer_name", None)
...
def _load_pi_processor_registry() -> None:
    ...
    import lerobot.policies.pi06.processor_pi06 as _pi06_processor  # noqa: F401
...
def load_policy_preprocessor_from_checkpoint(
    ...
    *,
    ...
    checkpoint_config = read_json(resolved_checkpoint_dir / "config.json")
    policy_type = str(checkpoint_config.get("type") or "").strip().lower()
    ...
    overrides = build_processor_overrides(
        ...
        override_tokenizer_name=policy_type not in {"pi06"},
    )
    try:
    ...
    return pipeline, {
        ...
        "device": device,
        "local_tokenizer_path": runtime["local_tokenizer_path"],
        "effective_tokenizer_name": resolve_pipeline_tokenizer_name(pipeline),
