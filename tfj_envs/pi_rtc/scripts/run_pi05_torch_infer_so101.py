...
class TorchChunkPolicyRuntime:
    ...
    def predict_action_chunk(self, batch: dict[str, torch.Tensor], **kwargs) -> torch.Tensor:
        ...
        with self._predict_grad_context(), self._autocast_context():
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Target control-loop fps. Defaults to --camera-fps when unset. "
            "Use this to slow robot actuation without forcing the camera driver to accept a lower fps."
        ),
    )
...
def main() -> int:
    ...
    if args.control_fps is not None and args.control_fps <= 0.0:
        raise ValueError(f"--control-fps must be > 0 when provided, got {args.control_fps}")
    ...
    control_fps = float(args.control_fps if args.control_fps is not None else args.camera_fps)
    ...
    try:
        ...
        recommended_min_steps = max(1, math.ceil(initial_chunk.total_time_s * control_fps))
        info(
            "Initial chunk ready: "
        ...
        current_threshold = estimate_prefetch_threshold(
            ...
            fallback_fps=float(control_fps),
        )
        info(
        ...
        while True:
            ...
            precise_sleep(max(1 / control_fps - dt_s, 0.0))
